...
def main():
    ...
    future_data_specific_x, future_data_specific_y = create_sequences(future_data_specific, features, target, seq_length=SEQUENCE_LENGTH)
    ...
    rmse = np.sqrt(mean_squared_error(pred2, future_data_specific_y))
    mae = mean_absolute_error(pred2, future_data_specific_y)
    print(f"Future single RMSE: {rmse}, MAE: {mae}")

    return
    env = OptionTradingEnv(
    future_data_specific, model, features, SEQUENCE_LENGTH, 
    min_max_scaler, target_standard_scaler
)

    agent_path = "option_pricing_agent_more_lstm"
