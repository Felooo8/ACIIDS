...
class OptionPricingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Ensure next_price corresponds to the same option contract (option_id)
        current_option_id = self.data.loc[self.current_step + self.sequence_length - 1, 'option_id']
        next_step = self.current_step + self.sequence_length
...
def main():
    ...
    model_path = 'models/option_pricing_model_new_12.keras'
    ...
    y_val_real = target_standard_scaler.inverse_transform(y_val.reshape(-1, 1))
    y_pred_real = target_standard_scaler.inverse_transform(y_pred.reshape(-1, 1))
