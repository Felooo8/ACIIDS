...
def plot_3d_predictions(options_test):
    ...
    ax.scatter(options_test['strike'], expiration_numeric, options_test['Actual Future Volatility'], 
               c='blue', label='Actual Future Volatility', marker='o', s=30)

    # Plot predicted future prices
    ax.scatter(options_test['strike'], expiration_numeric, options_test['Predicted Future Volatility'], 
               c='red', label='Predicted Future Volatility', marker='^', s=30)

    # Plot current prices
    ax.scatter(options_test['strike'], expiration_numeric, options_test['last'], 
               c='green', label='Current Volatility (at prediction time)', marker='s', s=30)

    # Set axis labels and title
    ax.set_xlabel('Strike Volatility')
    ax.set_ylabel('Expiration Date')
    ax.set_zlabel('Option Volatility')
    ax.set_title('3D Scatter Plot of Option Volatilities: Current, Predicted, and Actual')
...
def plot_predictions(Y_test, Y_pred):
    ...
    plt.title('Predicted vs Actual Future Option Volatilities')
    plt.legend()
    plt.show()
...
def predict_and_visualize():
    ...
    print(options_test[['date', 'strike', 'type', 'expiration', 'implied_volatility', 'Actual Future Volatility', 'Predicted Future Volatility']].head())
