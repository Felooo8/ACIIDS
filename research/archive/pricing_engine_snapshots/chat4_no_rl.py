...
def load_and_preprocess_data(market_data_path, text_data_path=None):
    ...
    data = data.groupby('option_id').filter(lambda x: len(x) >= 150 and x['price'].mean() > 30)
    ...
    # Save unscaled data
    data_unscaled = data[['Date', 'underlying_price', 'price_strike', 'iv', 'dte', 'price']].copy()
...
def main():
    ...
    model_path = 'models/option_pricing_model_asd33312.keras'
